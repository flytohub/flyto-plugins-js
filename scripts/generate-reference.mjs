...
const htmlSummaries = {
  ...
  "plugins/form-builder/ui/index.html#esc": "HTML-escape a dynamic value before string rendering.",
...
for (const relative of ["plugins/form-builder/ui/index.html", "plugins/image-crop/ui/index.html"]) {
  ...
  for (const match of html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/gi)) {
    const lineOffset = html.slice(0, match.index + match[0].indexOf(match[1])).split("\n").length - 1;
    declarations.push(...collectDeclarations(relative, match[1], lineOffset, htmlSummaries));
  }
